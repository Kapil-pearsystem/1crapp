...
@section('content')
    <div class="container-fluid">
         <!-- Page Heading -->
		    <div class="row mb-4">
                <div class="col-lg-4">
                    <h1 class="h3 mb-0 text-gray-800">End User List </h1>
                </div>
		     <!--   <div class="col-lg-8">-->
                        ...
                        <tbody>
                            @foreach ($users as $key=>$user)
                                <tr>
                                    ...
                                    <td>
        							     <label class="switch">
        								  <input id="market_status" onchange="updateStatus(this,{{ $user->id}})" type="checkbox"  @if($user->status==1) checked @endif>
        								  <small></small>
        								</label>
        							</td>
                                    ...
                                    ?>
									 <td>
									     <a href="#" data-message="{{ json_encode($user) }}" onclick="viewMessage(this)" class="btn btn-info bnt_alsss"  data-toggle="modal" data-target="#view_message"><i aria-hidden="true" class="fa fa-eye"></i></a>
  ...
  <div class="modal-dialog " role="document">
    <div class="modal-content">
      <div class="modal-body position-relative">
        ...
        <!-- Tab Content -->
         <h4 class="text-center text-info">End User List</h4><hr>
        <div class="tab-content p-3 enquiry_message" style="min-height:200px;">
        </div>
      </div>
    </div>
...
<div class="modal fade " id="next_step" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog " role="document">
    <div class="modal-content" style="border:2px solid blue;">
      <div class="modal-body position-relative">
        <!-- Close Button -->
        <button type="button" class="btn btn-primary rounded-circle border-2 position-absolute close-btn" data-dismiss="modal" onclick="closeModal()"  aria-label="Close"
          style="top: 10px; right: 10px; width: 40px; height: 40px;">
          <span aria-hidden="true">&times;</span>
        </button>
...
function closeModal(){
 ...
 function updateStatus(thiss,id) {
    var status = thiss.checked ? 1 : 0;
    $.ajax({
		type: "GET",
		cache: false,
		url: "{{url('end-users/status')}}/"+id+'/'+status,
