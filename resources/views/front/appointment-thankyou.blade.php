...
@if($data->custom_header_visible == 1)
...
@else
    ...
    <section class="tital_mg_cntss">
        ...
        <div class="midils_contnts">
            <div class="medilss">
                <h4>{!! ucwords($data->page_name) !!}</h4>
                <a href="{{ url('') }}">Home</a> &gt; <span>{!! ucwords($data->page_name) !!}</span>
            </div>
        </div>
    </section>
@endif
...
use App\Models\BookingEventModel;
    $steps = BookingEventModel::where('status', 1)->where('step', 3)->first();         
    $step4 = BookingEventModel::where('status', 1)->where('step', 4)->first();
@endphp
...
<!--- End YOUR CALL IS RESERVED! ---->	
@if($data->custom_footer_visible == 1)
