...
<section class="dash_board_pages">
 <div class="container">
 <form action="{{ route('feedback.submit') }}" method="POST" id="onecrappfeedback">
    @csrf
 <div class="h_titless">
        ...
        <table class="table table-bordered tst_cam_listst">
            ...
            <tbody>
                <tr>
                    <td class="red_bg">
                        <div class="input-box">
                            <label> <input type="radio" id="form-user1" name="overall_experience" value="1" class="form-radio" /> 1 </label>
                        </div>
                    </td>

                    <td class="yl_bg">
                        <div class="input-box">
                            <label> <input type="radio" id="form-user2" name="overall_experience" value="2" class="form-radio" /> 2 </label>
                        </div>
                    </td>

                    <td class="gr_l_bg">
                        <div class="input-box">
                            <label> <input type="radio" id="form-user3" name="overall_experience" value="3" class="form-radio" /> 3 </label>
                        </div>
                    </td>

                    <td class="gr_d_bg">
                        <div class="input-box">
                            <label> <input type="radio" id="form-user4" name="overall_experience" value="4" class="form-radio" /> 4 </label>
                        </div>
                    </td>
                </tr>
            </tbody>
...
!-- Recommend rating (0-10) -->
<input type="hidden" name="recommend_rating" id="recommend_rating">

<!-- Easy to manage (1-7) -->
<input type="hidden" name="easy_to_manage" id="easy_to_manage">
  ...
  <div class="slt_arara_2">
   ...
   <div class="form-group">
     <textarea name="suggestion" id="" cols="6" rows="6" class="form-control" placeholder="Please enter your suggestion for further improvement" required=""></textarea>
   </div>
...
<script>
 $('.form-radio').change(function() {
    ...
    let val = $(this).val(); // 1,2,3,4 selected rating

    // Sab hide karo
    $(".onepart, .twopart, .threepart, .frrepart").hide();

    // Jo input select hua hai, wo show karo
    if(val == 1){
        $(".onepart").show();
    }
    if(val == 2){
...
<script>
 $(".startMe").click(function(){ 
    ...
    $(".scoreNow").html(value);
    $("#recommend_rating").val(value); // Save in hidden input for Laravel
...
$(".startMe1").click(function(){ 
    ...
    $("#easy_to_manage").val(value); // Save in hidden input for Laravel
