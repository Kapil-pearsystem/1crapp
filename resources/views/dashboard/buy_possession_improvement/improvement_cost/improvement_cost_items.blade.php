    ...
    foreach($ItemImprovementCost as $itemPA){
        ...
        <div class="form-group managsss1" id="IC{{$itemPA->id}}">
            ...
            <span class="edttt bth_alsss">
                <a data-toggle="modal" data-target="#improvementCostModalEdit" onclick="editIC(<?=$itemPA->id?>,<?=$itemPA->prop_id?>,'<?=$itemPA->paid_name?>','<?=$itemPA->paid_type?>',<?=$itemPA->paid_amount?>,'<?=$itemPA->paid_percentOf?>','<?=$itemPA->paid_date?>',<?=$itemPA->paid_inLoan?>);" style="cursor: pointer;"><img src="{{url('')}}/img/edit.png" alt="edit" /></a>
...
<div class="btm_coststs">
    ...
    <span class="pricss">{{$currency}}
        ...
        <span id="totalImprovementCostPercentHidden" style="display: none;"><?=$percentOfPrice?></span>
    </span>
</div>