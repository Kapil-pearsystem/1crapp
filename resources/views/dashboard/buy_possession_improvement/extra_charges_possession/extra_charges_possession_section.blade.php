...
<div class="all_frm_list">
    ...
    <div class="form-group managsss1">
        ...
        <div class="both_areaa">
            <div class="lft_araeassd" style="width: 80%;">
                <input type="text" placeholder="Total Extra Charges" id="totalextraChargePercentInput" value="<?=$MainProperty->prop_extraChargePercent?>" name="totalextraChargePercentInput" autocomplete=off <?php if(count($ItemExtraCharge)){ ?> readonly <?php } ?>/>
                <span class="pursntss">% Of Price</span>
            </div>
            <div class="ri_lft_araeassd" style="width: 20%;margin-top: -25px;">
                <label>Itemize:</label>
                <div class="yesNoButton">
                    <input type="checkbox" class="toggle" id="toggle11" autocomplete="off" <?php if(count($ItemExtraCharge)){ echo "checked";} ?> name="extraChargesitemizedToggle"/>
                    <label for="toggle11">
    ...
    <div class="itm_cost_prch" id="itemisedPaidAmount" <?php if(!count($ItemExtraCharge)){ echo 'style="display: none;"';} ?>>
        ...
        <div class="all_frm_list" style="padding:0;" id="extra_charges_possession_items_content">
            @include('dashboard.buy_possession_improvement.extra_charges_possession.extra_charges_possession_items', ['ItemExtraCharge'=>ExtraChargesPossession($MainProperty)])
        </div>
    </div>
</div>