@php
    ...
    $percentOfPrice = $ItemExtraCharge['percentOfPrice'];
    ...
    foreach($ItemExtraCharge as $itemPA){
        ...
        <div class="form-group managsss1" id="IPA{{$itemPA->id}}">
            ...
            <span class="edttt bth_alsss">
                <a data-toggle="modal" data-target="#paidAmountModalEdit" onclick="editIPA(<?=$itemPA->id?>,<?=$itemPA->prop_id?>,'<?=$itemPA->paid_name?>','<?=$itemPA->paid_type?>',<?=$itemPA->paid_amount?>,'<?=$itemPA->paid_percentOf?>','<?=$itemPA->paid_date?>',<?=$itemPA->paid_inLoan?>);" style="cursor: pointer;"><img src="{{url('')}}/img/edit.png" alt="edit" /></a>
...
<div class="btm_coststs">
    ...
    <span class="pricss">
        ...
        <span id="totalExtraChargesPercentHidden" style="display: none;"><?=$percentOfPrice?></span>
    </span>
</div>