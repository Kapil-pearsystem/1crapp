    ...
    <div class="form-group managsss1">
        ...
        <div class="both_areaa">
            <div class="lft_araeassd" style="width: 80%;">
                <input type="text" placeholder="Total Conveyance Deed Cost" id="totalConvDeedPInput" value="<?=$MainProperty->prop_ConvDeedPercent?>" name="totalConvDeedPInput" autocomplete=off <?php if(count($ItemConvDeed)){ ?> readonly <?php } ?>/>
                <span class="pursntss">% Of Price</span>
            </div>
            <div class="ri_lft_araeassd" style="width: 20%;margin-top: -25px;">
                <label>Itemize:</label>
                <div class="yesNoButton">
                    <input type="checkbox" class="toggle" id="toggle3" autocomplete="off" <?php if(count($ItemConvDeed)){ echo "checked";} ?> name="cdrCostitemizedToggle"/>
                    <label for="toggle3">
