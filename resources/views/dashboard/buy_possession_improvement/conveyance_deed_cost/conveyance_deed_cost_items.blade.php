...
foreach($ItemConvDeedData as $itemPA){
    ...
    <div class="form-group managsss1" id="CDC{{$itemPA->id}}">
        ...
        <span class="edttt bth_alsss">
            <a  data-toggle="modal" data-target="#ConvDeedModalEdit" onclick="editCDC(<?=$itemPA->id?>,<?=$itemPA->prop_id?>,'<?=$itemPA->paid_name?>','<?=$itemPA->paid_type?>',<?=$itemPA->paid_amount?>,'<?=$itemPA->paid_percentOf?>','<?=$itemPA->paid_date?>',<?=$itemPA->paid_inLoan?>);" style="cursor: pointer;"><img src="{{url('')}}/img/edit.png" alt="edit" /></a>
...
<span class="pricss">
    ...
    <span id="totalItemizedConvDeed">{{ $ItemConvDeed['TotalAmount'] }}</span>
        <span id="totalConvDeedPercentHidden" style="display: none;"><?=$ItemConvDeed['percentOfPrice']?></span>
</span>
</div>