...
class SalesCompsARVEstimatesController extends Controller
{
    ...
    public function index($category, $id){
        ...
        $propertyListView = (string)view('sales-comps-arv.property_list',compact('propertyList','userSubscription'));

        $averageSalePriceSection = (string)view('sales-comps-arv.average-sale-price-section',compact('propertyList','MainProperty'));
        return view('sales-comps-arv.index',compact('MainProperty','propertyListView','averageSalePriceSection'));
    }
    ...
    public function listSortBy(Request $request){
        ...
        if($request->sortBy == 'Price'){
            $propertyList = $propertyList->orderBy('prop_salePrice','DESC');
        }elseif($request->sortBy == 'Sold'){
            $propertyList = $propertyList->orderBy('prop_dateOfSale','DESC');
        }
        $propertyList = $propertyList->get();
        $propertyList = $this->propertListCollect($propertyList,$MainProperty);
    ...
    private function propertListCollect($propertyList,$MainProperty){
        ...
        foreach ($propertyList as $key => $value) {
            ...
            $singlePropertyCollect->put('prop_salePrice',$value->prop_salePrice);
