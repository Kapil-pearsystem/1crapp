...
class AppointmentsController extends Controller
{
    public function index($page)
    {
        ...
        $booking = AppointmentBookingModel::where(['status'=>1, 'slug'=>$page])->first();
        if(is_null($booking)){
            abort(404);
        }
        // dd($booking);
    ...
    public function homework($page)
    {
        ...
        // dd($page);  ,'created_by'=>app('currentAgent')->id
        $homework = AppointmentBookingHomeworkModel::select('*','tbl_filedrive.title as file_name', 'tbl_filedrive.path as file_path')
        ->leftjoin('tbl_filedrive', 'tbl_filedrive.id','=','tbl_appointment_booking_homework.file_drive_id')
        ->where(['tbl_appointment_booking_homework.status'=>1, 'tbl_appointment_booking_homework.slug'=>$page])->first();
        if(is_null($homework)){
            abort(404);
        }
        ...
        $form_data = view('front.form-embeded',compact('data','cdos','p_services'))->render();
    ...
    public function thankyou($page){
        ...
        ->where('tbl_appointment_thankyou.slug', $page)->first();
        if(is_null($data)){
            abort(404);
        }
